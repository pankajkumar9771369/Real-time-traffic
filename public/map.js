var apiKey = "0duEFJnYzGflR3ABvtFBJWdOJAnuZSdR";
var centerCoords = [78.9629, 20.5937]; // General center of India
...
//     searchPosition=result.data.result.position
//     map.flyTo({
//         center: result.data.result.position,
//         speed: 3
//     });
//     new tt.Marker()
//         .setLngLat(searchPosition)
//         .addTo(map);
// }

function onSearchBoxResult(result) {
    // Properly getting the position of the search result
    var searchPosition = result.data.result.position;
    var placeName = result.data.result.address.freeformAddress || "Unnamed Location"; // Assuming the place name is available

    // Fly to the searched position
    map.flyTo({
        center: searchPosition,
        speed: 3
    });

    // Create a new marker at the searched position
    var marker = new tt.Marker()
        .setLngLat(searchPosition)
        .addTo(map);

    // Add a click event listener on the marker to show the name in a popup
    marker.setPopup(new tt.Popup({ offset: 35 })  // Adjust popup position if needed
        .setHTML("<h3>" + placeName + "</h3>"))    // Show place name
        .addTo(map);
}
